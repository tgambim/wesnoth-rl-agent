--modified version of Wesnoth generic recruit engine (https://github.com/wesnoth/wesnoth/blob/master/data/ai/lua/generic_recruit_engine.lua)
...
return {
    ...
    init = function(ai_cas, params)
        ...
        function do_recruit_eval(data, leader)
            -- Check if leader is on keep
            -- local leader = wesnoth.get_units { side = wesnoth.current.side, canrecruit = 'yes' }[1]

            if not leader then
                leader = wesnoth.get_units { side = wesnoth.current.side, canrecruit = 'yes' }[1]
            end
            ...
            local score = 0
            if can_recruit_best(leader) then
                score = 170000 -- default score if one not provided. Same as RCA AI
            end
        ...
        function ai_cas:recruit_rushers_eval(leader)
            ...
            local score = do_recruit_eval(recruit_data, leader)
            if score == 0 then
        ...
        function can_recruit_best(leader)
            ...
            if wesnoth.unit_types[recruit_type].cost <= wesnoth.sides[wesnoth.current.side].gold then
                recruit_data.recruit.recruit_type = recruit_type
                return true
            else
                return false
            end
        ...
        function ai_cas:recruit_rushers_exec()
            local check = AH.checked_recruit(ai, recruit_data.recruit.recruit_type, recruit_data.recruit.best_hex[1], recruit_data.recruit.best_hex[2])
